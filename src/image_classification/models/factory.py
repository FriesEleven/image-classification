...
def build_model(config: ExperimentConfig) -> nn.Module:
    if config.model_type == "mobilenetv2":
        return BaseMobileNetV2()
    if config.model_type == "eca":
        return ECAMobileNetV2()
    if config.model_type == "cbam":
        return CBAMMobileNetV2(aux_positions=config.aux_positions)
    if config.model_type == "se":
        return SEMobileNetV2(aux_positions=config.aux_positions)
