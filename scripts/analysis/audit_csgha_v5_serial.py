"""Audit exactly the completed CSGHA-v5 serial-s1 manifest without training or test use."""
...
DEFAULT_MANIFEST = Path(
    "artifacts/sweeps/cifar10_csgha_v5_rms_matched_serial_s1_20260831_150114/manifest.json"
)
DEFAULT_SNAPSHOT = Path("artifacts/audits/2026-08-31-csgha-v5-serial-s1/snapshot.json")
DEFAULT_OUTPUT = Path("reports/audits/2026-08-31-csgha-v5-serial-s1")
SMALL_FILES = (
...
def collect(manifest_path: Path) -> dict:
    ...
    if manifest.get("sweep_name") != "cifar10_csgha_v5_rms_matched_serial_s1":
        raise ValueError("Select the exact v5 serial-s1 manifest")
    if manifest.get("status") != "completed" or manifest.get("concurrent_jobs") != 1:
        raise ValueError("v5 serial-s1 manifest is not a completed serial sweep")
    if len(manifest.get("runs", [])) != 6:
        raise ValueError("v5 serial-s1 manifest must contain exactly six runs")
    runs = []
    for manifest_run in manifest["runs"]:
        run_id = manifest_run["experiment_id"]
        if "_serial_s1_seed" not in run_id:
            raise ValueError(f"Unexpected serial-s1 ID: {run_id}")
    ...
    return {
        ...
        "scope": "Exact v5 serial-s1 manifest only; no rms1/rms2/rms3, training, or test evaluation.",
    }
...
def analyze(snapshot: dict) -> dict:
    ...
    groups = {}
    for model_type in ("hybrid_leaky", "csgha_v5"):
    ...
    guided = {row["seed"]: row for row in valid_rows if row["model_type"] == "csgha_v5"}
    deltas = [guided[seed]["best_validation_percent"] - controls[seed]["best_validation_percent"] for seed in (42, 43, 44)]
    paired = {
        "comparison": "csgha_v5 - hybrid_leaky", "unit": "percentage_points",
...
def write_outputs(snapshot: dict, snapshot_path: Path, output: Path) -> dict:
    ...
    lines += [
        ...
        "", f"v5 胜出 {result['paired']['wins']}/3；配对均值为负，不能声称优于matched control。", "",
        "## 完整性与协议", "",
        ...
        f"Control参数量 {control['parameters_total']:,}；v5参数量 {guided['parameters_total']:,}（+{guided['parameters_total'] - control['parameters_total']:,}）。FLOPs为项目解析估计，不是profiler实测。", "",
        "## 历史参照边界", "",
        "旧v3、Independent middle、Independent shallow来自2026-08-30审计，只作历史参照；本批control与v4 retry1 control的三个best checkpoint哈希完全一致。", "",
        "## 下一步", "", "进入版本匹配checkpoint诊断：先严格复现完整5k validation，再做guidance置零/置换/训练均值及deep置零干预，确认RMS是否降低饱和以及输入相关贡献是否被削弱。", "",
