"""P8 v3: deterministic FP32 repair of the P8 v2 TF32 boundary failure."""
...
from scripts.analysis.benchmark_early_exit_p8_v2 import validate_protocol as validate_p8_v2_protocol

PROTOCOL = ROOT / "reports/experiments/2026-09-05-early-exit-p8-v3-design/protocol_manifest.json"
P8_V2_PROTOCOL = ROOT / "reports/experiments/2026-09-05-early-exit-p8-v2-design/protocol_manifest.json"


def validate_protocol() -> dict:
    validate_p8_v2_protocol()
    p = read_json(PROTOCOL)
    if p["status"] != "frozen_before_p8_v3_execution":
        raise ValueError("P8 v3 protocol not frozen")
    if sha256(P8_V2_PROTOCOL) != p["parent_protocol_sha256"]:
        raise ValueError("P8 v2 parent protocol changed")
    for receipt in p["failed_p8_v2_receipts"]:
        if sha256(ROOT / receipt["path"]) != receipt["sha256"]:
            raise ValueError(f"P8 v2 failure receipt changed: {receipt['path']}")
    assert p["expected_raw_files"] == 1200
    return p


def configure_numerics() -> None:
    """Use true FP32 convolution and deterministic kernels for audited timing."""
...
def check_routing(model, batches, threshold, *, rtol=1e-4, atol=1e-5) -> dict:
    checked = 0
    route_errors = prediction_errors = logit_tolerance_errors = 0
    max_abs_logit_difference = max_tolerance_ratio = 0.0
    with torch.inference_mode():
        for batch in batches[:32]:
            ...
            reference_logits = torch.where(early[:, None], early_logits, final_logits)
            expected = reference_logits.argmax(1)
            absolute = (logits - reference_logits).abs()
            tolerance = atol + rtol * reference_logits.abs()
            close = absolute <= tolerance
            route_errors += int(((paths == 0) != early).sum())
            prediction_errors += int((logits.argmax(1) != expected).sum())
            logit_tolerance_errors += int((~close.all(1)).sum())
            max_abs_logit_difference = max(max_abs_logit_difference, float(absolute.max()))
            max_tolerance_ratio = max(max_tolerance_ratio, float((absolute / tolerance).max()))
            checked += len(batch)
    return {
        "samples_checked": checked,
        "route_errors": route_errors,
        "prediction_errors": prediction_errors,
        "logit_tolerance_errors": logit_tolerance_errors,
        "logit_rtol": rtol,
        "logit_atol": atol,
...
def execute(output: Path) -> dict:
    p = validate_protocol()
    configure_numerics()
    expected_numerics = {k: p["numerics"][k] for k in numerical_settings()}
    if numerical_settings() != expected_numerics:
        raise RuntimeError("P8 v3 numerical backend configuration mismatch")
    ...
    provenance.update(
        ...
        matmul_allow_tf32=torch.backends.cuda.matmul.allow_tf32,
        deterministic_algorithms=torch.are_deterministic_algorithms_enabled(),
        numerical_repair="TF32 disabled; deterministic FP32 convolution required by P8 v3 protocol",
            ...
            for seed in matrix["seeds"]:
                ...
                for batch_size in p["batch_sizes"]:
                    for round_index in range(p["rounds"]):
                        ...
                        checks = check_routing(
                            model,
                            batches,
                            threshold,
                            rtol=p["numerics"]["logit_rtol"],
                            atol=p["numerics"]["logit_atol"],
                        )
                        (output / "checks" / f"{key}.json").write_text(json.dumps(checks) + "\n")
                        if checks["route_errors"] or checks["logit_tolerance_errors"]:
                        ...
                        print(f"[P8 v3] {key} routes={early_fraction:.4f} actual_saving={saving:.4f}", flush=True)
    ...
    result = {"status": "completed", "schema_version": 3, "raw_files": len(rows),
    ...
    if len(rows) != p["expected_raw_files"] or not result["source_unchanged"]:
        raise RuntimeError("Incomplete or mixed-source P8 v3")
    (output / "p8_v3_results.json").write_text(json.dumps(result, indent=2) + "\n")
    return result
