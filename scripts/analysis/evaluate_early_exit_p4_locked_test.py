"""Evaluate the hash-locked P4 policy on CIFAR-100 official test exactly once."""
...
from scripts.analysis.freeze_early_exit_p4_test import EVALUATOR, TEST_GATES
from scripts.launch_early_exit_p4 import SEEDS

EMPTY_SHA256 = "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
DEFAULT_LOCK = Path("reports/experiments/2026-09-03-early-exit-p4-cifar100/official_test_lock.json")
DEFAULT_OUTPUT = Path("artifacts/official_tests/early_exit_p4_cifar100_20260903")
DEFAULT_REPORT = Path("reports/experiments/2026-09-03-early-exit-p4-cifar100-test")


def locked_test_gates(seed_results: list[dict]) -> dict[str, bool]:
    if [row["seed"] for row in seed_results] != list(SEEDS):
        raise ValueError("P4 test gates require ordered seeds 66 through 68")
    ...
    return {
        ...
        "each_worst_class_accuracy_drop_at_most_0_04": all(
...
def _require_versioned_clean_inputs(paths: tuple[Path, ...]) -> dict:
    ...
    if runtime.get("git_status") != "":
        raise RuntimeError("P4 method-locked test requires a clean Git working tree")
    if runtime.get("tracked_source_diff_sha256") != EMPTY_SHA256:
        raise RuntimeError("P4 method-locked test requires no tracked source diff")
    for path in paths:
        ...
        if result.returncode != 0:
            raise RuntimeError(f"P4 lock input must be committed before test access: {relative}")
    return runtime
...
def verify_lock(lock_path: Path) -> tuple[dict, dict, dict, dict, dict]:
    ...
    if lock.get("status") != "ready_for_one_shot_method_locked_cifar100_test":
        raise ValueError("P4 lock did not authorize test evaluation")
    if lock.get("dataset") != "cifar100" or lock.get("training_seeds") != list(SEEDS):
        raise ValueError("P4 lock references a different dataset or cohort")
    if lock.get("test_confirmation_gates") != TEST_GATES:
        raise ValueError("P4 test gates differ from the preregistered evaluator")
    if lock.get("evaluator") != str(EVALUATOR.relative_to(ROOT)):
        raise ValueError("P4 lock references a different evaluator")
    if lock.get("evaluator_sha256") != _sha256(EVALUATOR):
        raise ValueError("P4 evaluator differs from the frozen lock")
    if lock.get("p3_boundary_preserved") != {
        "p3_status": "stop_without_test",
        "p3_official_test_accessed": False,
    }:
        raise ValueError("P4 lock does not preserve the P3 test boundary")

    manifest_path = ROOT / lock["manifest"]
    audit_path = ROOT / lock["audit"]
    confirmation_path = ROOT / lock["confirmation"]
    policy_path = ROOT / lock["policy_lock"]
    expected_hashes = {
        manifest_path: lock["manifest_sha256"],
        audit_path: lock["audit_sha256"],
        confirmation_path: lock["confirmation_sha256"],
        policy_path: lock["policy_lock_sha256"],
    }
    for path, expected in expected_hashes.items():
        if _sha256(path) != expected:
            raise ValueError(f"P4 locked input hash mismatch: {path}")
    audit = json.loads(audit_path.read_text(encoding="utf-8"))
    confirmation = json.loads(confirmation_path.read_text(encoding="utf-8"))
    policy_lock = json.loads(policy_path.read_text(encoding="utf-8"))
    if audit.get("issues") != {}:
        raise ValueError("P4 locked audit contains issues")
    if confirmation.get("status") != "ready_for_method_locked_cifar100_test":
        raise ValueError("P4 locked confirmation is not accepted")
    if not confirmation.get("gates") or not all(confirmation["gates"].values()):
        raise ValueError("P4 locked confirmation has a failed gate")
    policy = confirmation.get("locked_policy")
    if not isinstance(policy, dict):
        raise TypeError("P4 confirmation omits the locked policy")
    ...
    for key in (
        ...
        if policy.get(key) != lock_policy.get(key):
            raise ValueError(f"P4 confirmation/lock policy mismatch: {key}")
    if policy.get("threshold_candidates_considered_on_p4") != 0:
        raise ValueError("P4 confirmation considered threshold candidates")
    if policy.get("per_model_recalibration") is not False:
        raise ValueError("P4 models were recalibrated")
    if policy_lock.get("p3_frozen_result", {}).get("official_test_accessed") is not False:
        raise ValueError("P3 official-test boundary was not preserved")

    _manifest, runs, _split_fingerprint = load_manifest(manifest_path)
    checkpoint_hashes = lock.get("best_checkpoint_sha256", {})
    if checkpoint_hashes != confirmation.get("best_checkpoint_sha256"):
        raise ValueError("P4 lock checkpoint map differs from confirmation")
    for run in runs.values():
        ...
        if _sha256(checkpoint) != checkpoint_hashes.get(experiment_id):
            raise ValueError(f"P4 locked checkpoint mismatch: {experiment_id}")
...
def _markdown(result: dict) -> str:
    ...
    for value in result["seed_results"]:
        ...
        rows.append(
            "| {seed} | {baseline:.2f} | {final:.2f} | {locked:.2f} | {early:.2f} | "
            "{saving:.2f} | {drop:+.2f} | {worst:+.2f} | {changed}/{harmed}/{rescued} |".format(
                seed=value["seed"],
...
def evaluate_once(
    ...
    lock, _audit, confirmation, runs, paths = verify_lock(lock_path)
    lock_sha256 = _sha256(lock_path)
    if access_registry is None:
        access_registry = (
            ROOT / "artifacts/official_test_access_registry" / f"early_exit_p4_cifar100_{lock_sha256[:16]}"
        )
    ...
    for path in (output, report, start_marker, complete_marker):
        if path.exists():
            raise FileExistsError(f"P4 CIFAR-100 one-shot guard already exists: {path}")

    runtime = _require_versioned_clean_inputs(
        (lock_path, paths["audit"], paths["confirmation"], paths["policy_lock"], EVALUATOR)
    )
    started_at = datetime.now(timezone.utc).isoformat()
    start_record = {
        "schema_version": 1,
        "status": "p4_method_locked_cifar100_test_started",
        "started_at_utc": started_at,
        "lock": str(lock_path.relative_to(ROOT)),
        "lock_sha256": lock_sha256,
        "manifest_sha256": lock["manifest_sha256"],
        "audit_sha256": lock["audit_sha256"],
        "confirmation_sha256": lock["confirmation_sha256"],
        "policy_lock_sha256": lock["policy_lock_sha256"],
        "evaluator_sha256": lock["evaluator_sha256"],
        "runtime_commit": runtime["git_commit"],
        "output": str(output),
        "report": str(report),
        "rule": "This marker permanently forbids a second P4 CIFAR-100 test evaluation.",
    }
    ...
    policy = confirmation["locked_policy"]
    ...
    for seed in SEEDS:
        ...
        if not np.array_equal(baseline_labels, multi_labels):
            raise ValueError(f"Matched CIFAR-100 test order differs for P4 seed {seed}")
        if len(baseline_values) != 1 or len(multi_values) != 3:
            raise ValueError(f"Unexpected P4 test model output count for seed {seed}")
        ...
        if labels_fingerprint is None:
            ...
            raise ValueError("CIFAR-100 test label order differs across P4 models")
    ...
    status = "paper_evidence_complete" if all(gates.values()) else "p4_test_boundary_failure"
    raw_test = ROOT / "data/cifar-100-python/test"
    result = {
        "schema_version": 1,
        "status": status,
        "scope": (
            "One method-locked CIFAR-100 official-test evaluation of six P4 best checkpoints "
            "and threshold 0.903; no test-time tuning."
        ),
        "started_at_utc": started_at,
        ...
        "aggregate": {"all_seeds": _aggregate(seed_results)},
        "gates": gates,
        ...
        "limitations": [
            ...
            "The 4pp class-risk boundary was developed after the frozen P3 failure.",
            "MAC saving is an operation-count proxy rather than new hardware latency.",
            "Both training datasets use MobileNetV2.",
        ],
        "recommended_next_step": (
            "Stop new training and proceed to frozen paper tables, figures and writing."
            if status == "paper_evidence_complete"
            else "Archive the boundary result without retuning or rerunning; write the limitation."
        ),
    }
    ...
    source_index = {
        ...
        "confirmation": {
            ...
            "sha256": lock["confirmation_sha256"],
        },
        "policy_lock": {"path": lock["policy_lock"], "sha256": lock["policy_lock_sha256"]},
        "evaluator": {"path": lock["evaluator"], "sha256": lock["evaluator_sha256"]},
        "test_results": {"path": str(result_path.relative_to(ROOT)), "sha256": _sha256(result_path)},
        "prediction_files": prediction_files,
        "access_started": {"path": str(start_marker.relative_to(ROOT)), "sha256": _sha256(start_marker)},
    }
    ...
    complete_record = {
        **start_record,
        "status": "p4_method_locked_cifar100_test_completed",
...
def main() -> int:
    ...
    if args.verify_only:
        lock, _audit, confirmation, _runs, _paths = verify_lock(lock_path)
        print(
            json.dumps(
                {
                    "status": lock["status"],
