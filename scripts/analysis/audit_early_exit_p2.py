"""Create an immutable, manifest-driven audit of the completed P2a transfer batch."""
...
def collect(manifest_path: Path, launcher_log_path: Path) -> dict:
    ...
    if manifest.get("sweep_name") != EXPECTED_SWEEP_NAME:
        raise ValueError("Select the exact early-exit P2a transfer manifest")
    if manifest.get("status") != "completed" or manifest.get("concurrent_jobs") != 1:
        raise ValueError("Early-exit P2a manifest is not a completed serial sweep")
    if len(manifest.get("runs", [])) != 6:
        raise ValueError("Early-exit P2a manifest must contain exactly six runs")

    runs = []
    for manifest_run in manifest["runs"]:
        run_id = manifest_run["experiment_id"]
        if "_p2a_seed" not in run_id:
            raise ValueError(f"Unexpected P2a ID: {run_id}")
    ...
    return {
        ...
        "scope": (
            "Exact completed P2a manifest only; unseen training seeds 57/58/59; "
            "validation/transfer subsets only; no official-test evaluation."
        ),
    }
...
def analyze(snapshot: dict) -> dict:
    ...
    if not launcher_log.get("exists"):
        ...
        if "Sweep status: completed" not in launcher_text:
            global_issues.append("launcher_log_not_completed")
        if "exit8 threshold=0.984" not in launcher_text:
            global_issues.append("launcher_log_missing_frozen_policy")
        if EXPECTED_SOURCE_SELECTION_SHA256 not in launcher_text:
            global_issues.append("launcher_log_missing_source_policy_hash")
    ...
    for entry in snapshot["runs"]:
        ...
        if seed not in EXPECTED_SEEDS or manifest_run.get("seed") != seed:
    ...
    expected_matrix = {(model_type, seed) for model_type in EXPECTED_ARCHITECTURES for seed in EXPECTED_SEEDS}
    ...
    if set(controls) != set(EXPECTED_SEEDS) or set(exits) != set(EXPECTED_SEEDS):
...
def write_outputs(snapshot: dict, snapshot_path: Path, output: Path) -> dict:
    ...
    if snapshot_path.exists() or output.exists():
        raise FileExistsError("P2 audit evidence is immutable; choose fresh output paths")
    ...
    lines = [
        "# Early-exit P2a formal audit",
        "",
        "This report audits only the completed serial P2a manifest; official test data are excluded.",
        "",
