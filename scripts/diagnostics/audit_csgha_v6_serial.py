"""Run version-matched v6/control checkpoint diagnostics; never train or open test data."""
...
DEFAULT_AUDIT = Path("reports/audits/2026-09-01-csgha-v6-serial-v6s1/audit_results.json")
DEFAULT_OUTPUT = Path("artifacts/diagnostics/csgha_v6_serial_information_20260901_v1")
DIAGNOSTIC_SOURCES = (
    "scripts/diagnostics/audit_csgha_v6_serial.py",
...
def selected_rows(audit: dict) -> list[dict]:
    ...
    expected = Counter((model, seed) for model in ("hybrid_leaky", "csgha_v6") for seed in (42, 43, 44))
...
def main() -> int:
    ...
    plan = [{
        ...
        "conditions": ["original", "deep_zero"] + (
            ...
            if row["model_type"] == "csgha_v6" else []
        ),
    } for row in rows]
    preflight = []
    for row in rows:
