"""Run version-matched v4/control checkpoint diagnostics; never train or open test data."""
...
DEFAULT_AUDIT = Path("reports/audits/2026-08-31-csgha-v4-retry1/audit_results.json")
DEFAULT_OUTPUT = Path("artifacts/diagnostics/csgha_v4_retry1_information_20260831_v1")
DIAGNOSTIC_SOURCES = (
    "scripts/diagnostics/audit_csgha_v4_retry1.py",
...
def selected_rows(audit: dict) -> list[dict]:
    ...
    expected = Counter((model, seed) for model in ("hybrid_leaky", "csgha_v4") for seed in (42, 43, 44))
...
def main() -> int:
    ...
    plan = [{
        ...
        "conditions": ["original", "deep_zero"] + (
            ...
            if row["model_type"] == "csgha_v4" else []
        ),
    } for row in rows]
    preflight = []
    for row in rows:
