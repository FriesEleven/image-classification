"""Launch P8 v2 after validating the immutable input evidence."""
...
from scripts.analysis.benchmark_early_exit_p8_v2 import PROTOCOL, validate_protocol
...
def main():
    ...
    for line in processes.splitlines():
        ...
        if len(parts) == 3 and parts[1].startswith("python") and any(
            name in parts[2] for name in ("scripts/train.py", "run_baselines.py", "benchmark_early_exit_p8.py", "benchmark_early_exit_p8_v2.py")
        ):
    ...
    with (artifacts / "early_exit_p8_launcher.lock").open("a") as lock:
        ...
        output = artifacts / "analyses" / f"early_exit_p8_v2_{stamp}"
        log = artifacts / "launcher_logs" / f"early_exit_p8_v2_{stamp}.log"
        command = [sys.executable, str(ROOT / "scripts/analysis/benchmark_early_exit_p8_v2.py"), "--output", str(output)]
