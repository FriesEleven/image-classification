"""Launch the serial early-exit P1 disjoint-development protocol."""
...
def validated_plan(experiment_tag: str = EXPERIMENT_TAG):
    ...
    for run in plan:
        ...
        if any(config[key] != value for key, value in protocol.items()):
            raise ValueError(f"Unexpected early-exit P1 protocol: {run['experiment_id']}")
        if run["seed"] not in SEEDS or config["seed"] != run["seed"]:
            raise ValueError(f"Unexpected P1 training seed: {run['experiment_id']}")
        model_type = config["model_type"]
        if model_type not in {"mobilenetv2", "multi_exit"}:
            raise ValueError(f"Unexpected P1 model: {run['experiment_id']}")
        if model_type == "multi_exit":
            ...
            if any(config[key] != value for key, value in expected.items()):
                raise ValueError(f"Unexpected P1 multi-exit contract: {run['experiment_id']}")
        counts[(model_type, run["seed"])] += 1
        if not config["experiment_name"].endswith(f"_{experiment_tag}_seed{run['seed']}"):
            raise ValueError("P1 requires a fresh tagged experiment ID")
    expected_counts = Counter(
        {(model_type, seed): 1 for model_type in ("mobilenetv2", "multi_exit") for seed in SEEDS}
    )
    if counts != expected_counts:
        raise ValueError("Expected exactly 2 matched models x training seeds 54/55/56")
    return plan
...
def main() -> int:
    ...
    if args.dry_run:
        print(
            ...
            "40k/5k/5k, fixed split, validation/calibration-only; no training started."
        )
        return 0

    ARTIFACTS_DIR.mkdir(parents=True, exist_ok=True)
    with (ARTIFACTS_DIR / "early_exit_p1_launcher.lock").open("a") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as error:
            raise RuntimeError("Early-exit P1 is already running") from error
        ...
        log_path = log_directory / f"early_exit_p1_serial_{args.experiment_tag}_{timestamp}.log"
        ...
        print(f"Early-exit P1 started with PID {process.pid}")
        print(f"Matrix: {len(plan)} runs; concurrency={JOBS}; training_seeds={SEEDS}")
