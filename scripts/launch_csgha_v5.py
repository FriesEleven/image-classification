"""Launch six validation-only CSGHA-v5 / matched-control experiments."""
...
SWEEP = PROJECT_ROOT / "configs/sweeps/csgha_v5_matched_serial.yaml"
EXPERIMENT_TAG = "s1"
JOBS = 1
...
def validated_plan(experiment_tag: str = EXPERIMENT_TAG):
    ...
    expected = Counter(
        {(variant, seed): 1 for variant in ("hybrid_leaky", "csgha_v5") for seed in (42, 43, 44)}
    )
    if counts != expected:
    ...
    for run in plan:
        ...
        if not config["experiment_name"].endswith(f"_serial_{experiment_tag}_seed{run['seed']}"):
            raise ValueError("CSGHA v5 requires fixed new experiment IDs")
    return plan
...
def main():
    ...
    with (ARTIFACTS_DIR / "csgha_v5_launcher.lock").open("a") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as error:
            raise RuntimeError("CSGHA v5 sweep is already running") from error
        ...
        log_path = log_directory / f"csgha_v5_rms_matched_serial_{args.experiment_tag}_{timestamp}.log"
        ...
        print(f"CSGHA v5 matched sweep started with PID {process.pid}")
