"""Launch six serial validation-only CSGHA-v6 / matched-control experiments."""
...
SWEEP = PROJECT_ROOT / "configs/sweeps/csgha_v6_matched_serial.yaml"
EXPERIMENT_TAG = "v6s1"
JOBS = 1
...
def validated_plan(experiment_tag: str = EXPERIMENT_TAG):
    ...
    expected = Counter(
        {(variant, seed): 1 for variant in ("hybrid_leaky", "csgha_v6") for seed in (42, 43, 44)}
    )
    if counts != expected:
    ...
    for run in plan:
        ...
        if not config["experiment_name"].endswith(f"_serial_{experiment_tag}_seed{run['seed']}"):
            raise ValueError("CSGHA v6 requires fixed new experiment IDs")
    return plan
...
def main():
    ...
    if args.dry_run:
        print(f"Validated: 6 new {args.experiment_tag} runs, {JOBS} concurrent jobs, from scratch, validation-only; no training started.")
        return 0
    ARTIFACTS_DIR.mkdir(parents=True, exist_ok=True)
    with (ARTIFACTS_DIR / "csgha_v6_launcher.lock").open("a") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as error:
            raise RuntimeError("CSGHA v6 sweep is already running") from error
        ...
        log_path = log_directory / f"csgha_v6_capped_matched_serial_{args.experiment_tag}_{timestamp}.log"
        ...
        print(f"CSGHA v6 matched sweep started with PID {process.pid}")
